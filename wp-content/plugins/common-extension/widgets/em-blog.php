 ...
 * @since Layers 1.0.0
 */
if( class_exists('Layers_Widget') && !class_exists( 'em_blog' ) ) {
	class em_blog extends Layers_Widget {
		...
		*/
		function __construct() {
			...
			*/
			$this->widget_title = __( 'EM Blog Widget' , 'common' );
			$this->widget_id = 'em_blog';
			$this->post_type = 'post';
			...
			$this->defaults = array (
				'title' => __( 'Our <span>Blog</span>', 'common' ),
		...
		*/
		function widget( $args, $instance ) {
			...
			*/
			...
			$widget_container_class[] = 'em-blog-widget';
			...
			<div id="<?php echo esc_attr( $widget_id ); ?>" class="<?php echo esc_attr( $widget_container_class ); ?>" <?php $this->selective_refresh_atts( $args ); ?>>
				...
				<?php if( $post_query->have_posts() ) { ?>
					...
					<?php if( 'style1' == $this->check_and_return( $instance, 'text_style' ) ){?>	
						<div class="em40_row blog-messonary">
							<?php while ($post_query->have_posts()) : $post_query->the_post(); 
								...
								<div class=" grid-item <?php echo $post_column_class; ?>  col-xs-12 col-sm-6"  data-cols="<?php echo $col_count; ?>" >
									<!-- BLOG QUERY -->
					...
					<?php }elseif( 'style2' == $this->check_and_return( $instance, 'text_style' )){?>
						...
						<div class="em40_row">
						<div class="owl-carousel blog_carousel curosel-style">
							<?php while ($post_query->have_posts()) : $post_query->the_post(); 

								$post_column_class = array();
								$post_column_class[] = 'thumbnail';
								$post_column_class[] = ( 'on' != $this->check_and_return( $instance, 'design', 'gutter' ) ? 'nospace' : 'extra-blog' );
								$post_column_class[] = ( '' != $this->check_and_return( $instance, 'design' , 'column-text-align' ) ? $this->check_and_return( $instance, 'design' , 'column-text-align' ) : 'text-left'  ) ;
								$post_column_class[] = ( '' != $this->check_and_return( $instance, 'design', 'column-background-color' ) && 'dark' == layers_is_light_or_dark( $this->check_and_return( $instance, 'design', 'column-background-color' ) ) ? 'invert' : '' );
								$post_column_class = implode( ' ' , $post_column_class );					
								...
								<div class="<?php echo $post_column_class; ?>  col-md-12" >
									<!-- BLOG QUERY -->
									<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
										<!-- SINGLE BLOG -->											
											<div class="common-single-blog">													
